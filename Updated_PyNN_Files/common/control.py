...
def build_run(simulator):
    def run_until(time_point, run_from_steady_state=False, callbacks=None):
        ...
        if callbacks:
            ...
            simulator.state.run_until(time_point, run_from_steady_state=run_from_steady_state)
    ...
    def run(simtime, run_from_steady_state=False, callbacks=None):
        ...
        """
        return run_until(simulator.state.t + simtime,
                         run_from_steady_state=run_from_steady_state,
                         callbacks=callbacks)
